...
function SidebarTrigger({
  ...
  return (
    <Button
      ...
      variant="outlined"
      size="icon"
      className={cn("size-7 border-none hover:bg-transparent", className)}
      onClick={(event) => {
        onClick?.(event);
        toggleSidebar();
      }}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md px-2 py-1.5 text-left text-sm outline-hidden ring-sidebar-ring transition-[width,height,padding] hover:bg-muted hover:text-foreground active:bg-muted active:text-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-muted data-[active=true]:font-medium data-[active=true]:text-foreground data-[state=open]:hover:bg-muted data-[state=open]:hover:text-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-4 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "hover:bg-muted hover:text-foreground",
        outline: "bg-background hover:bg-muted hover:text-foreground ",
      },
      size: {
        default: "h-8 text-sm",
        sm: "h-7 text-xs",
